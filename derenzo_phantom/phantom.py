...
class DerenzoPhantom(object):
    ...
    def export_to_G4gps_macro(self, fname, num_events, gamma_en, source_name, activity,
        ...
        with open(fname, 'w') as fh:
            for section in self.sections:
                ...
                source_name_sec = source_name + \
                    str(round(section.well_sep))+'mm'
                i = 0
                for xy in section.locs:
                    source_name_sec_cyl = source_name_sec+str(i)
                    activity_sec_cyl = section.r*section.r*100
                    export_to_G4mac(fh, xy[0], xy[1], 0, section.r, gamma_en,
                                    ne, source_name_sec_cyl, activity_sec_cyl, halfz=self.depth)
                    i += 1
                    sum_activity += activity_sec_cyl
        with open(fname, 'a') as fh:
            fh.write('# the total activity is %.2f Bq\n' % (sum_activity))
...
if __name__ == "__main__":
    ...
    source_name = 'derenzo'
    activity = 100
    my_phantom = DerenzoPhantom(radius, well_seps, cyl_height=cyl_height)
    my_phantom.show()
    my_phantom.export_to_G4gps_macro('derenzo.mac', 1e6, 661.657, source_name, activity,
                                     event_mode='equal_activity')
