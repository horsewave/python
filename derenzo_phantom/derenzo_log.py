...
def export_to_G4mac(filehandle, x, y, z, r, energy, num_evs, source_name, activity, halfz=None):
    """
    Write Derenzo wells to a Geant4 macro file for use with 
    G4GeneralParticleSource.
    """
    srcstr = '/gate/source/addSource   %s \n' % (source_name)
    srcstr += '/gate/source/%s/setActivity   %.2f becquerel \n' % (
        source_name, activity)
    srcstr += '/gate/source/%s/gps/particle  gamma \n' % (source_name)
    srcstr += '/gate/source/%s/gps/energytype Mono \n' % (source_name)
    srcstr += '/gate/source/%s/gps/setType backtoback \n' % (source_name)
    srcstr += '/gate/source/%s/gps/monoenergy 0.511 MeV \n' % (source_name)
    # 2D vs 3D
    if halfz is None:
        srcstr += '/gate/source/%s/gps/type Plane\n' % (source_name)
        srcstr += '/gate/source/%s/gps/shape Circle \n' % (source_name)
    else:
        srcstr += '/gate/source/%s/gps/type Volume\n' % (source_name)
        srcstr += '/gate/source/%s/gps/shape Cylinder \n' % (source_name)
    srcstr += '/gate/source/%s/gps/centre %.2f %.2f %.2f mm\n' % (
        source_name, x, y, z)
    srcstr += '/gate/source/%s/gps/radius %.2f mm\n' % (source_name, r)
    if halfz is not None:
        srcstr += '/gate/source/%s/gps/halfz  %.2f mm\n' % (source_name, halfz)
    srcstr += '/gate/source/%s/gps/angtype  iso \n' % (source_name)
    srcstr += '/gate/source/%s/visualize 2000 red 3 \n' % (
        source_name)
    srcstr += '/gate/source/list \n\n'
    ...
    with open('derenzo.mac', 'w') as fh:
        for i, fs in enumerate(feature_sizes):
            ...
            for xy in locs:
                ...
                ax.add_patch(cyl)
                export_to_G4mac(fh, xy[0], xy[1], 0, r,
                                gamma_en, num_evs, halfz=None)
